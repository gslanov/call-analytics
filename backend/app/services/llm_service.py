...
Операторы совершают ИСХОДЯЩИЕ звонки клиентам, чтобы подтвердить уже оформленные заказы.
...
1. introduced_self — Оператор представился клиенту в начале звонка. КОНТЕКСТ: это ИСХОДЯЩИЙ звонок — оператор сам звонит клиенту подтвердить заказ и ЗАРАНЕЕ ЗНАЕТ имя клиента из заказа. Поэтому в приветствии может прозвучать И имя оператора, И имя клиента — не путай их (как различить — см. пункт «а» ниже и addressed_by_name).
   Засчитывай true если выполнено ХОТЯ БЫ ОДНО:
   а) оператор назвал своё имя. Имена операторов: Галина, Александра, Анна, Анастасия (ASR искажает: «Калина», «Лина», «Анестасия», «Аннастасия», «Сашина» и т.п.). ВАЖНО: звонок исходящий, оператор заранее знает имя клиента из заказа и может в приветствии назвать И своё имя, И имя клиента. Имя из набора операторов (Галина/Александра/Анна/Анастасия) — это представление оператора; ДРУГОЕ имя в приветствии — это обращение к клиенту (учитывается в addressed_by_name), НЕ считай его именем оператора;
...
1. addressed_by_name — Оператор обращался к клиенту по имени хотя бы раз. ВАЖНО: это ИСХОДЯЩИЙ звонок — оператор заранее знает имя клиента из заказа и ЧАСТО называет его уже в приветствии: «Здравствуйте, Андрей», «Андрей, добрый день, это Пироги №1», «Меня зовут Галина, звоню Андрею по поводу заказа». Засчитывай true при ЛЮБОМ обращении к клиенту по имени, в том числе в самом начале разговора. Не путай с именем самого оператора: если в приветствии звучат ДВА имени и одно из них из набора операторов (Галина/Александра/Анна/Анастасия) — то имя оператора это оно, а второе имя — обращение к клиенту (засчитывай true).
