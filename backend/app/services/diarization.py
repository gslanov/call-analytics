...
class DiarizationService:
    ...
    def _diarize_mono_llm(
        ...
        prompt = (
            ...
            "- ОПЕРАТОР начинает первым: приветствие, название компании (\"Пироги №1\", \"Компания Пироги\")\n"
            "- ОПЕРАТОР спрашивает адрес, время доставки, состав заказа, предлагает доп. товары\n"
            "- ОПЕРАТОР называет цены, суммы, подтверждает заказ, говорит \"Записала\", \"Поставила\"\n"
            "- КЛИЕНТ отвечает на вопросы, называет адрес, выбирает товары, спрашивает цены\n"
            "- КЛИЕНТ обычно короче отвечает, а оператор задаёт вопросы и ведёт разговор\n"
            "- Если фраза начинается с \"Да\", \"Нет\", \"Ага\" после вопроса оператора — это КЛИЕНТ\n"
            "- Если фраза содержит профессиональные термины (\"килограммовые\", \"доставка\", \"бонусы\") — вероятно ОПЕРАТОР\n\n"
