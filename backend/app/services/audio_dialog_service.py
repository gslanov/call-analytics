...
ЗАДАЧА: убрать из РАЗМЕТКИ то, чего в записи не было. И только это.

╔══════════════════════════════════════════════════════════════════════╗
║ ЭТАЛОН нужен ТОЛЬКО чтобы проверить: звучала такая реплика или нет.  ║
║ ТЕКСТ всегда берётся из РАЗМЕТКИ — она распознаёт русскую речь       ║
║ ТОЧНЕЕ эталона. НЕ заменяй слова РАЗМЕТКИ словами ЭТАЛОНА.           ║
╚══════════════════════════════════════════════════════════════════════╝

Правила:
1. Таймкоды, роли и ФОРМУЛИРОВКИ бери из РАЗМЕТКИ — дословно, как есть.
2. Удаляй реплику РАЗМЕТКИ, только если в ЭТАЛОНЕ нет НИЧЕГО похожего на
   неё — ни этих слов, ни этого смысла. Это признак выдумки.
3. Расхождения в словах — НЕ повод ничего менять. Эталон часто перевирает
   названия, имена, суммы и адреса (телефонная запись низкого качества);
   РАЗМЕТКА в таких местах надёжнее. Оставляй вариант РАЗМЕТКИ.
4. Ничего не переписывай, не сокращай и не «улучшай». Реплика либо остаётся
   дословно как в РАЗМЕТКЕ, либо удаляется целиком.
5. Ничего не добавляй от себя.
6. Если ЭТАЛОН пуст или в нём нет человеческой речи (гудки, тишина, фразы
...
class AudioDialogService:
    ...
    def crosscheck(
        ...
        *,
        ...
        # Сверке позволено ТОЛЬКО удалять реплики. Если она переписала текст —
        # значит подставила формулировки эталона, а он на телефонном 8 kHz
        # врёт в названиях, именах и суммах («Пироги №1» → «Тракен-Один»,
        # «5 620» → «620»). Такой результат хуже исходного, откатываем.
        if not _keeps_wording(segments, dialog.segments):
            logger.warning(
                "Сверка переписала формулировки вместо удаления реплик — откат "
                "к разметке Gemini"
            )
            return dialog
...
def _keeps_wording(
    result: list[AudioDialogSegment],
    original: list[AudioDialogSegment],
    *,
    min_similarity: float = 0.75,
    min_share_kept: float = 0.8,
) -> bool:
    """Проверяет, что сверка только удаляла реплики, а не переписывала их.

    Для каждой реплики результата ищем самую похожую в исходной разметке.
    Если большинство реплик там не находится — значит текст подменили
    формулировками эталона, и результат хуже исходника.
    """
    import difflib
    ...
    originals = [s.text for s in original]
    kept = 0
    for seg in result:
        best = max(
            (difflib.SequenceMatcher(None, seg.text, o).ratio() for o in originals),
            default=0.0,
        )
        if best >= min_similarity:
            kept += 1
    return (kept / len(result)) >= min_share_kept
