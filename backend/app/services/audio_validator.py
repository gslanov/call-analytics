...
def validate_audio_file_path(
    ...
    *,
    ...
    """Validate audio file already on disk (streaming-friendly variant).

    Args:
        filename: Original filename (for extension check).
        file_path: Path to file on disk.
        file_hash: Pre-computed SHA-256 (incrementally during upload streaming).
        file_size: File size in bytes (already known from streaming).
        existing_hashes: Set of hashes already in DB (for dedup).
    """
    ...
    if file_size > settings.max_file_size_bytes:
        return ValidationResult(
            valid=False,
            error=f"Размер файла ({file_size // 1024 // 1024} MB) превышает лимит {settings.max_file_size_mb} MB",
        )

    if file_size == 0:
        return ValidationResult(valid=False, error="Файл пустой")

    # Magic bytes check — read first 16 bytes from disk
    with open(file_path, "rb") as f:
        head = f.read(16)
    if not _check_magic_bytes(head, ext):
        return ValidationResult(
            valid=False,
            error=f"Содержимое файла не соответствует расширению {ext}",
        )

    duration, channels, probe_error = _probe_audio(file_path)
    if probe_error:
