...
1. introduced_self — Оператор представился клиенту в начале звонка. КОНТЕКСТ: это ИСХОДЯЩИЙ звонок (оператор сам звонит клиенту, чтобы подтвердить заказ), оператор НЕ ЗНАЕТ имя клиента в начале разговора. Поэтому любое имя в приветствии — это имя ОПЕРАТОРА, а не клиента.
   Засчитывай true если выполнено ХОТЯ БЫ ОДНО:
   а) оператор назвал своё имя. Типичные имена: Галина, Александра, Анна, Анастасия. ASR часто искажает русские имена — «Калина», «Лина», «Анестасия», «Аннастасия», «Александра», «Сашина» и т.п. — любое слово в позиции имени в первой реплике = имя оператора;
...
8. offered_upsell — Оператор предложил клиенту КУПИТЬ дополнительный ПРОДУКТ за деньги (апсейл): напитки (морс, сок, лимонад, чай), дополнительные пироги, десерты, выпечку, подарочные товары.
   ❌ НЕ ЗАСЧИТЫВАЕТСЯ как апсейл сервисный вопрос про инвентарь/аксессуары к заказу:
   - «Нужны ли нож или одноразовая посуда?» — это сервис, не продажа
   - «Нужны салфетки/столовые приборы?» — то же самое
   - «Хватит ли пирогов на компанию?» — это уточнение количества уже заказанного, не апсейл
   ✅ ЗАСЧИТЫВАЕТСЯ:
   - «Не хотите взять морс?» → true
   - «Может быть к десерту пирожок?» → true
   - «У нас сейчас акция на сладкое, добавим?» → true
   Ключевой признак апсейла: оператор предлагает доплатить за дополнительный товар. Если не про оплату — это не апсейл.
9. explained_upsell_benefit — Оператор рассказал про выгоду апсейла (null если апсейл не предлагался, т.е. offered_upsell=false). Засчитывай true если оператор: назвал что входит в продукт, упомянул что популярно/часто берут, сказал к чему подходит («на большие мероприятия берут», «у нас есть сырная, мясная, овощная»). Также true если оператор объяснил состав/выгоду подарочного продукта (например, описал что входит в подарок ко дню рождения и условия его получения).
