...
class Settings(BaseSettings):
    ...
    # Сверка с независимой расшифровкой (страховка от выдумок Gemini).
    # Один проход gpt-4o-transcribe даёт дословный текст без ролей, Gemini
    # сверяет с ним свою разметку и вычищает реплики, которых в записи не было.
    # Стоит ~$0.011 за звонок сверху (итого ~$0.021 против $0.008 без сверки).
    # Требует рабочего баланса OpenAI; без него сверка молча пропускается.
    audio_dialog_crosscheck: bool = True
